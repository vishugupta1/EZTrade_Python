...
def perform_analysis(symbol="", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 1, 1), sv=100000):
    ...
    portfolio_dataframe = pd.DataFrame(index=stocks_data.index, columns=[symbol+"_BM",symbol+"_SL"])
    ...
    benchmark_initial_price = stocks_data.iloc[0, 0]
    ...
    for date in stocks_data.index:
        benchmark_current_price = stocks_data.loc[date, 'Adj Close']
    ...
    # portfolio_dataframe.bfill()
    # portfolio_dataframe.ffill()
    portfolio_dataframe.loc[:, symbol+"_SL"] = portfolio_dataframe.loc[:, symbol+"_SL"]/portfolio_dataframe.iloc[0, 1]
    # #portfolio_dataframe.loc[:, symbol+"_MS"] = portfolio_dataframe.loc[:, symbol+"_MS"]/portfolio_dataframe.loc[sd, symbol+"_MS"]
    portfolio_dataframe.loc[:, symbol+"_BM"] = portfolio_dataframe.loc[:, symbol+"_BM"]/portfolio_dataframe.iloc[0, 0]
...
if __name__ == "__main__":
    perform_analysis(symbol="IBM", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 1, 1), sv=100000) # insample
    #perform_analysis(symbol="IBM", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 1, 1), sv=100000) # outsample
